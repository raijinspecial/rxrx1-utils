...
url_base_path = 'gs://rxrx1-us-central1/tfrecords/by_exp_plate_site-42'
...
def main(use_tpu,
    ...
    test_files = rxinput.get_tfrecord_names(url_base, test_df)
    # Get transforms
    ...
    test_files = rxinput.get_tfrecord_names(url_base, test_df, sites=sites)
    all_files = rxinput.get_tfrecord_names(url_base, train_df, sites=sites)

    classifier_pred = tf.contrib.tpu.TPUEstimator(
        use_tpu=pred_on_tpu,
        model_fn=train_model_fn,
        params=PRED_INPUT_FN_PARAMS,
        config=config,
        train_batch_size=train_batch_size,
        eval_batch_size=train_batch_size,
...
#         all_files = dummy_pad_files(train_glob, test_glob, pred_batch_size)
    ...
    classifier_pred.export_saved_model(os.path.join(model_dir, 'saved_model'), serving_input_receiver_fn)
    ...
    return {
        ...
        'test_predict': classifier_pred.predict(input_fn=test_input_fn, yield_single_examples=True),
